...
import Bin.Cliente;
import Bin.Venda;
import Janela.Financeiro.Detalhamento.JFrameVenda;
import Janela.Financeiro.Operacional.JFrameEspecificarRecebimento;
...
public class JPanelPendencias extends JPanel implements
	...
	private JPanel painalGrafico = new JPanelGrafico();
	...
	private void especificar() {
		...
		try {
			...
			if (venda.getCliente() != 1) {
				JFrameEspecificarRecebimento dt = new JFrameEspecificarRecebimento(
						venda);
				dt.setVisible(true);
				atualizarTabelaContas();
	...
	private void detalhe() {
		...
		try {
			...
			Venda venda = (Venda) banco.buscarPorId(Venda.class, id);
			JFrameVenda dt = new JFrameVenda(
					venda);
			dt.setVisible(true);
			atualizarTabelaContas();
		} catch (ArrayIndexOutOfBoundsException e) {
